$(function() {
  ...
  function refresh() {
    $.get('http://exceptional-realty-property-ad.herokuapp.com/properties.json', function(response){
      ...
      Property.all = []; //clear array of property instances
      $('table').find('tbody').empty() //empty table body

      $.each(response, function(i, property){
        var property = new Property(property.street, property.city, property.state, 
          property.price, property.posted);
      });
...
setInterval(refresh, 5000); // in milliseconds
  //http://exceptional-realty-property-ad.herokuapp.com/properties
  //http://exceptional-realty-property-ad.herokuapp.com/properties.json
